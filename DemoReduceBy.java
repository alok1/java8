...
public class DemoReduceBy {
	...
	System.out.println("Total value:: "+ reducedvalue.get());
	 Double reducedvaluewithidentity = studentList.stream().map(Student::getMark).reduce(0d,Double::sum); //intial value is 0 and converted to decimal
	 System.out.println("Total value with initial offset:: "+reducedvaluewithidentity);
	 
	 Double reducedvalueincrementedby10 = studentList.stream().map(Student::getMark).reduce(10d,Double::sum); //intial value is 0 and converted to decimal
	 System.out.println("Total value with initial offset 10:: "+reducedvalueincrementedby10);
	 
	
	 
	// internal working of combiner
